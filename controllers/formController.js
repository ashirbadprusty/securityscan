...
export const createForm = async (req, res) => {
  try {
    ...
    if (!name || !email || !phone || !reason || !file) {
      return res
        .status(400)
        .json({ message: "All required fields must be filled!" });
    }

    // Ensure `personToMeet` is an object
    let personToMeetData;
    try {
      personToMeetData =
        typeof personToMeet === "string"
          ? JSON.parse(personToMeet)
          : personToMeet;
    } catch (error) {
      return res.status(400).json({ message: "Invalid personToMeet format!" });
    }

    if (
      !personToMeetData ||
      typeof personToMeetData !== "object" ||
      !personToMeetData._id
    ) {
      return res.status(400).json({ message: "Invalid personToMeet data!" });
    }
            ...
            $dateFromString: {
              dateString: {
                $concat: [
                  "$date", "T", 
                  { $substrCP: ["$timeTo", 0, 2] }, ":", 
                  { $substrCP: ["$timeTo", 3, 2] }, ":00Z" // Ensure UTC time
                ]
              },
